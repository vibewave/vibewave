...
const YouTubeSearch = () => {
	...
	const convertPtToSec = pt => {
		...
		const hoursToSec =
			hIndex !== -1 ? parseInt(pt.substr(tIndex + 1, mIndex), 10) * 60 * 60 : 0;
		const minutesToSec =
			mIndex !== -1 ? parseInt(pt.substr(tIndex + 1, mIndex), 10) * 60 : 0;
		const seconds =
			sIndex !== -1 ? parseInt(pt.substr(mIndex + 1, sIndex), 10) : 0;
	...
	const handleSubmit = async event => {
		...
		console.log('search on front end is ', search);
		const filteredVideoList = await axios.post('/youtube/search', {
			search: search,
		});
		...
		console.log(filteredVideoList.data);
		setSearchResult(filteredVideoList.data);
	};
	...
	const chooseVideo = async video => {
		...
		await dispatch(
			addVideo(
				videoDetails,
				roomId
				// convertPtToSec(videoDetails.contentDetails.duration)
			)
		);
